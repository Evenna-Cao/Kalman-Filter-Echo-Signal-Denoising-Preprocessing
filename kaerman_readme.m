...
% 文件内容结构
% 
% processedResults(frameIndex) = struct(...
%     'frameCount',       % 帧序号 (整数)
%     'MTD_after',        % 杂波抑制后的RD数据 (复数矩阵)
%     'Vr',               % 多普勒速度轴 (向量)
%     'Range_plot',       % 距离轴 (向量)
%     'predictedPosition',% 预测位置 [距离单元索引, 多普勒单元索引]
%     'detectedPosition'  % 检测位置 [距离单元索引, 多普勒单元索引]
% );
% 
% 
% 1. 加载数据文件
% 
% % 加载处理结果
% load('processedResults.mat', 'processedResults');
% % 获取总帧数
% numFrames = length(processedResults);
% 
% 
% 
% 
...
% 
% % 获取第5帧数据
% frame5 = processedResults(5);
% 
% % 提取具体数据
% frameCount = frame5.frameCount;
% MTD_after = frame5.MTD_after;          % 杂波抑制后的RD数据
% Vr = frame5.Vr;                        % 多普勒速度轴
% Range_plot = frame5.Range_plot;        % 距离轴
% predictedPos = frame5.predictedPosition; % 预测位置坐标
% detectedPos = frame5.detectedPosition; % 检测位置坐标
% 
% 
% 
% 
...
% 
% % 创建新图窗
% figure('Name', sprintf('帧号: %d', frameCount), ...
%        'Position', [100, 100, 800, 600]);
% 
% % 显示RD图
% imagesc(Vr, Range_plot, db(abs(MTD_after)));
% colorbar;
% xlabel('多普勒速度 (m/s)');
% ylabel('距离 (m)');
% title(sprintf('杂波抑制后RD图 (帧号: %d)', frameCount));
% axis xy; % 确保距离轴方向正确
% 
% % 标记预测位置和检测位置
% hold on;
% plot(Vr(predictedPos(2)), Range_plot(predictedPos(1)), 'bo', ...
%     'MarkerSize', 10, 'LineWidth', 1.5, 'DisplayName', '预测位置');
% plot(Vr(detectedPos(2)), Range_plot(detectedPos(1)), 'r*', ...
%     'MarkerSize', 10, 'LineWidth', 1.5, 'DisplayName', '检测位置');
...
% 
% % 提取所有检测位置
% allDetectedPositions = zeros(numFrames, 2);
% for i = 1:numFrames
%     allDetectedPositions(i, :) = [processedResults(i).Range_plot(processedResults(i).detectedPosition(1)), ...
%                                   processedResults(i).Vr(processedResults(i).detectedPosition(2))];
% end
% 
% % 绘制目标轨迹
% figure;
% plot(allDetectedPositions(:, 2), allDetectedPositions(:, 1), 'r-*');
% xlabel('多普勒速度 (m/s)');
% ylabel('距离 (m)');
% title('目标运动轨迹');
% grid on;
...
% 
% % 计算预测误差
% predictionErrors = zeros(numFrames, 1);
% for i = 1:numFrames
%     frame = processedResults(i);
%     predRange = frame.Range_plot(frame.predictedPosition(1));
%     detRange = frame.Range_plot(frame.detectedPosition(1));
%     predVel = frame.Vr(frame.predictedPosition(2));
%     detVel = frame.Vr(frame.detectedPosition(2));
% 
%     predictionErrors(i) = sqrt((predRange - detRange)^2 + (predVel - detVel)^2);
% end
% 
% % 绘制预测误差
% figure;
% plot(1:numFrames, predictionErrors, 'b-o');
% xlabel('帧号');
% ylabel('预测误差 (m)');
% title('卡尔曼滤波预测误差');
% grid on;
...
% 
%     % 获取目标位置
%     rangeIdx = frame.detectedPosition(1);
%     dopplerIdx = frame.detectedPosition(2);
% 
%     % 提取目标特征
%     targetFeatures(i).frame = frame.frameCount;
%     targetFeatures(i).range = frame.Range_plot(rangeIdx);
%     targetFeatures(i).velocity = frame.Vr(dopplerIdx);
%     targetFeatures(i).amplitude = abs(frame.MTD_after(rangeIdx, dopplerIdx));
% 
%     % 计算信噪比 (简化版)
%     localArea = frame.MTD_after(max(1, rangeIdx-5):min(size(frame.MTD_after,1), rangeIdx+5), ...
%                               max(1, dopplerIdx-5):min(size(frame.MTD_after,2), dopplerIdx+5));
%     noisePower = mean(abs(localArea(:)));
%     targetFeatures(i).snr = 20*log10(targetFeatures(i).amplitude / noisePower);
% end
% 
% % 转换为表格便于分析
% targetTable = struct2table(targetFeatures);
% disp(targetTable);
...
for fileIdx = 1:length(rawFiles)
    try
        ...
        %% 初始化卡尔曼滤波器
        initialState = [trackData.R(1); trackData.AZ(1); 0; 0];
